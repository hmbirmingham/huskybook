...
| exact_location | text | private, room number or precise meeting spot. Never sent to the browse endpoint. Only included in a requester's own request list once that specific request is `accepted` |
...
| verified | integer (0/1) | a separate, higher trust bar than just having a signed-in account (e.g. a manual review pass), unused, see BUILD_LOG |
...
| requester_user_id | integer, fk → users.id | who actually sent it, and this, not requester_name, is what's checked |
| note | text | optional |
...
| display_name | text | set once, right after first sign-in, and it's what other people see you as |
| created_at | text | ISO timestamp |
...
What's still not here: no email verification beyond "you clicked the link" (no re-confirmation, no account recovery flow), no rate limiting beyond the one-link-per-minute cooldown on requesting a new sign-in link, and no admin/moderation tooling. See "Where it ended up" in BUILD_LOG.md.
